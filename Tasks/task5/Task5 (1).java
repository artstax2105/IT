...
//        System.out.println(sameVowelGroup("hoops", "chuff", "bot", "bottom"));
    ...
    public static StringBuilder numToEng(int number) {
        ...
        if (number / 100 > 0) result.append(String.format("%s hundred ", zeroToNineteen[number / 100]));
        if (number % 100 >= 20) result.append(String.format("%s ", decades[number % 100 / 10]));
        if (number % 10 > 0 || number == 0) result.append(zeroToNineteen[number % 20]);
        return result;
    }
